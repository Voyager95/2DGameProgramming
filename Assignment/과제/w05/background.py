...
class HorzScrollBackground:
    def __init__(self, imageName):
        self.imageName = imageName
        self.image = gfw.image.load(res(imageName))
        self.cw, self.ch = get_canvas_width(), get_canvas_height()  # cw, ch는 화면의 크기
        self.scroll = 0
        self.speed = 0
    ...
    def draw(self):
        left, bottom = 0, 0
        page = self.image.w * self.ch // self.image.h  # 세로로 확대했을 때, 가로의 크기가 얼마인지 보자는 것임
        curr = int(-self.scroll) % page

        if curr > 0:
            sw = int(1 + self.image.h * curr / self.ch)  # 이미지를 원래 비율로 보면
            sl = self.image.w - sw
            src = sl, 0, sw, self.image.h  # 표시할 이미지의 일부를 선택
            dw = int(sw * self.ch / self.image.h)  # Curr이랑 비슷한 값아녀
            dst = curr - dw, 0, dw, self.ch  # x,y,h,w
        ...
        if curr < self.cw:
            dw = self.cw - curr
            sl = int(1 + self.image.h * dw / self.ch)
            src = 0, 0, sl, self.image.h
            dw = int(sl * self.ch / self.image.h)
...
class VertScrollBackground:
    ...
    def draw(self):
        left, bottom = 0, 0
        page = self.image.h * self.cw // self.image.w  # 세로로 확대했을 때, 가로의 크기가 얼마인지 보자는 것임
        curr = int(-self.scroll) % page
        if curr > 0:
            sh = int(1 + self.image.w * curr / self.cw)  # 이미지를 원래 비율로 보면
            sl = self.image.h - sh
            src = 0, sl, self.image.h, sh  # 표시할 이미지의 일부를 선택
            dh = int(sh * self.cw / self.image.w)  # Curr이랑 비슷한 값아녀
            dst = 0, curr - dh, self.cw, dh  # x,y,h,w
            self.image.clip_draw_to_origin(*src, *dst)
        ...
        if curr < self.ch:
            dh = self.ch - curr
            sl = int(1 + self.image.w * dh / self.cw)
            src = 0, 0, self.image.w, sl
            dh = int(sl * self.cw / self.image.w)
